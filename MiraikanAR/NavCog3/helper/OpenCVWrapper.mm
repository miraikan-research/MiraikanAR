...
#include <opencv2/ArucoDetector.h>
...
static void detect(std::vector<std::vector<cv::Point2f> > &corners, std::vector<int> &ids, CVPixelBufferRef pixelBuffer) {
    cv::aruco::Dictionary dictionary = getPredefinedDictionary(cv::aruco::DICT_7X7_1000);
    ...
    detector.detectMarkers(mat, corners, ids, rejectedCandidates);
...
//    estimatePoseSingleMarkers(corners, markerSize, intrinMat, distCoeffs, rvecs, tvecs);
